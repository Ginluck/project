...
-(void)tapBtnAndcancelBtnClick{
    ...
    [UIView animateWithDuration:0.2 animations:^{
        ...
        {
            [self.delegate1 cancelBtnClick:titleAddress provinceId:self.titleIDMarr[0] cityId:@"" areaId:@""];
        }
        else if (self.titleIDMarr.count ==2)
        {
            [self.delegate1 cancelBtnClick:titleAddress provinceId:self.titleIDMarr[0] cityId:self.titleIDMarr[1] areaId:@""];
        }
        else if (self.titleIDMarr.count ==3)
        {
             [self.delegate1 cancelBtnClick:titleAddress provinceId:self.titleIDMarr[0] cityId:self.titleIDMarr[1] areaId:self.titleIDMarr[2]];
        }
...
-(void)setupAllTitle:(NSInteger)selectId{
    ...
    _lineLabel.backgroundColor = K_Prokect_MainColor;
    ...
    for (int i = 0; i < self.titleMarr.count ; i++) {
        ...
        [titleBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
         [titleBtn setTitleColor:K_Prokect_MainColor forState:UIControlStateSelected];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.titleIDMarr.count > tableView.tag){
        ...
        if (self.PCCTID == pcctId){
            [cell.nameLabel setTextColor:K_Prokect_MainColor];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (tableView.tag == 0 || tableView.tag == 1 || tableView.tag == 2){
        if (tableView.tag == 0){
            ProvinceModel *provinceModel = self.provinceMarr[indexPath.row];
            NSString * provinceID = provinceModel.code_p;
            //1. 修改选中ID
            ...
            CityModel * cityModel = self.cityMarr[indexPath.row];
            NSString * cityID = cityModel.code_c;
            ...
            CountyModel * countyModel = self.countyMarr[indexPath.row];
            NSString * countyID = countyModel.code_a;
...
-(void)getAddressMessageDataAddressID:(NSInteger)addressID  provinceIdOrCityId: (NSString *)provinceIdOrCityId{
    ...
    NSString * UserID =@"123";
        if (addressID == 1) {
            ...
            //获取市区的URL
            addressUrl = @"userApp/SELECT_CITY";
            //请求市区需要传递的参数
            parameters = @{@"code_p" : provinceIdOrCityId,
                           @"user_id" : UserID};
        }
        else if(addressID == 3){
            //获取县的URL
            addressUrl = @"userApp/SELECT_AREA";
            //请求县需要传递的参数
            parameters = @{@"code_c" : provinceIdOrCityId,
                           @"user_id" : UserID};
        }
    //网络请求
    [RequestHelp POST:addressUrl parameters:parameters success:^(id result) {
